...
export default function PostJobButton({ externalOpen, onExternalClose }: PostJobButtonProps = {}) {
  ...
  const addTodo    = () => setForm(f => ({ ...f, todoItems: [...f.todoItems, ''] }));
  const removeTodo = (i: number) => setForm(f => ({ ...f, todoItems: f.todoItems.filter((_, idx) => idx !== i) }));
  const updateTodo = (i: number, val: string) => setForm(f => ({ ...f, todoItems: f.todoItems.map((t, idx) => idx === i ? val : t) }));

  const handleBudgetChange = (raw: string) => {
    const digits = raw.replace(/\D/g, '');
    setForm(f => ({ ...f, budgetPerDay: digits ? Number(digits).toLocaleString('id-ID') : '' }));
  };
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          <div
            className="relative w-full max-w-sm bg-white rounded-t-3xl shadow-2xl flex flex-col"
          ...
          >
            ...
            {done ? (
              <div className="flex flex-col items-center justify-center gap-4 flex-1 px-6 py-12 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Orang di sekitarmu dapat melihat dan merespons permintaanmu.
                  </p>
                </div>
                <button
                  onClick={close}
              ...
              <form onSubmit={handleSubmit} className="overflow-y-auto flex flex-col gap-4 px-5 py-4 pb-8">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm placeholder:text-gray-300 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm placeholder:text-gray-300 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 resize-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="300.000"
                      className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm placeholder:text-gray-300 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                    />
                  </div>
                  <div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold text-gray-500">
                      Daftar Tugas <span className="font-normal text-gray-400">(opsional)</span>
                    </label>
                    <button
                      type="button"
                      onClick={addTodo}
                    ...
                    {form.todoItems.map((item, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm placeholder:text-gray-300 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                        />
                        {form.todoItems.length > 1 && (
                          <button
                            type="button"
                            onClick={() => removeTodo(i)}
                            className="w-8 h-8 shrink-0 flex items-center justify-center rounded-xl bg-red-50 text-red-400"
                          >
                            <Trash2 size={13} />
                          </button>
                        )}
