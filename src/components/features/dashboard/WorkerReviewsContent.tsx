...
import { useWorkerReviews, SortOption } from '@/hooks/useWorkerReviews';
...
function RatingSummary({ avg, total, dist }: { avg: number; total: number; dist: RatingDist[] }) {
...
export default function WorkerReviewsContent({ workerId, workerName }: Props) {
  const router   = useRouter();
  const sentinel = useRef<HTMLDivElement>(null);
  const [sortBy, setSortBy] = useState<SortOption>('recent');

  const {
    reviews, stats, isLoading, isError,
    isFetchingNextPage, fetchNextPage, hasNextPage,
  } = useWorkerReviews(workerId, sortBy);
        ...
        <div className="flex items-center gap-3 px-4 h-14">
          ...
          <div>
            <h1 className="text-base font-bold text-gray-900 leading-tight">
              Rating {workerName ?? 'Pelamar'}
            </h1>
            <p className="text-[11px] text-gray-400">Ulasan dari pelanggan sebelumnya</p>
          </div>
        </div>
      </div>
        ...
        {!isLoading && !isError && reviews.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Orang ini belum pernah mendapat ulasan.
            </p>
          </div>
        )}
